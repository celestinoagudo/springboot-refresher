...
public class DataGenerator {
    ...
    public static void generateStudents(
        ...
        for (int counter = 0; counter <= 49; ++counter) {
            ...
            final var email =
                    "%s%s@continental.edu".formatted(firstName, lastName).toLowerCase();
            ...
            final var firstEnrollmentId = new EnrollmentId(savedFirstCourse.getId(), savedStudent.getId());
            final var secondEnrollmentId = new EnrollmentId(savedSecondCourse.getId(), savedStudent.getId());
            final var thirdEnrollmentId = new EnrollmentId(savedThirdCourse.getId(), savedStudent.getId());

            final var firstEnrollment = new Enrollment(firstCourse, LocalDateTime.now(), firstEnrollmentId, student);
            final var secondEnrollment = new Enrollment(secondCourse, LocalDateTime.now(), secondEnrollmentId, student);
            final var thirdEnrollment = new Enrollment(thirdCourse, LocalDateTime.now(), thirdEnrollmentId, student);
